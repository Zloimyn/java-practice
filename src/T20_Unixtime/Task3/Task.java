...
public class Task {
    ...
    private static void printWorkHours(
        ...
        LocalDateTime startTime = LocalDateTime.of(START_YEAR,2,START_DAY,startHours,startMinutes);
        LocalDateTime endTime;
        for (int i = 1; i < shiftAmount; i++){ // цикл должен начинаться с 1, а количество итераций должно быть равно количеству смен
            endTime = LocalDateTime.of(START_YEAR,2,START_DAY,shiftContinuation + startHours,startMinutes); // вычислите дату и время окончания смены
            System.out.println("Cмена " + i + ". Начало: " + startTime + ", конец: " + endTime);
            startTime.plusHours(MAX_SHIFT);  // обновите переменную startTime
        }
    }
}
